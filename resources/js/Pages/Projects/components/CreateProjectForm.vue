...
const form = useForm({
  ...
  project_language: "",
  project_country: "",
  project_category: [], // Initialize as an array for multi-select
  team_id: "", // Initialize as an empty string
});
...
const submitForm = () => {
  const res = form.post(route("projects.store"));
...
<template>
  <FormSection @submitted="submitForm()">
    ...
    <template #description>
      Create a new project. There needs to be a team, in order to make a
      project. If no team is available, create a new
      <Link :href="route('teams.create')" class="text-indigo-600"
        >team first</Link
      >. There is a 1 to 1 relationship between teams and projects.
    </template>
